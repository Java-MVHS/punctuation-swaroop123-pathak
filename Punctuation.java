	 ...
	 */
	 ...
	 // call searc it from main
	 // searchIt : prints 3 blanck lines, and calls getpucntion()
	 // getpunctuation conatins the long string and calls check method
	 //check method checks the whole string and sends to print 
	 // print prints it out
	...
	"and I say, \"Come then, let us go forward together with our united strength.\"");
		...
		public String checkForPunctuation(String words)
		{
			...
			if(words.charAt(0) == '.' || words.charAt(0) == ';'|| words.charAt(0) == '!' || words.charAt(0) == ',' || words.charAt(0) == '?' || words.charAt(0) == '/' || words.charAt(0) == ':' || words.charAt(0) == '(' || words.charAt(0) == ')')
			{
				return words + " " ;
			} 
			else if(words.charAt(len-1) == '.' || words.charAt(len-1) == ';'|| words.charAt(len-1) == '!' || words.charAt(len-1) == ',' || words.charAt(len-1) == '?' || words.charAt(len-1) == '/' || words.charAt(len-1) == ':' || words.charAt(len-1) == '(' || words.charAt(len-1) == ')')
			{ 
				return words + " ";
			}
			else 
		...
		public void printWords(String words2)
		{
